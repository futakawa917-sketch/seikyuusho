...
### 5. 受信イベント起動（Gmail Push）

通常運用では、Gmail APIのPush通知をCloud Pub/SubとCloud Runで受け、
`repository_dispatch` によってGitHub Actionsを起動できます。

- Gmailの変更通知だけを受け取るため、メール本文はCloud Runへ送信しません
- Cloud Runは `snshack.online@gmail.com` の通知だけを許可します
- `watch` は毎朝9時に自動更新します
- Pub/SubからCloud Runへの通信はOIDC認証を使用します
- GitHubトークンはSecret Managerに保存します
